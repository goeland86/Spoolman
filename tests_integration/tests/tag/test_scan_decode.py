...
def test_scan_decodes_a_tigertag():
    ...
    assert decoded.get("material_type") is None
    assert decoded.get("brand_name") is None
...
def test_scan_decodes_a_qidi_tag():
    ...
    assert decoded.get("net_weight_g") is None
    assert decoded.get("diameter_mm") is None
